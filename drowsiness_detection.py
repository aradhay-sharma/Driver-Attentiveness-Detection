...
while True: 
	# Extract a frame 
	frame = vs.read()
	cv2.putText(frame, "PRESS 'q' TO QUIT", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 3) 
	# Resize the frame 
	...
	for (i, rect) in enumerate(rects): 
		...
		if EAR < EAR_THRESHOLD: 
			...
			if FRAME_COUNT >= CONSECUTIVE_FRAMES: 
				...
				cv2.putText(frame, "STAY ALERT!", (270, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
		else: 
			if FRAME_COUNT >= CONSECUTIVE_FRAMES: 
	...
	for i in ts:
		total_ts.append(i)
	#display the frame 
	cv2.imshow("Driver Attentiveness Camera", frame)
...
plt.title('EAR & MAR Calculation via Webcam')
plt.ylabel('EAR & MAR')
